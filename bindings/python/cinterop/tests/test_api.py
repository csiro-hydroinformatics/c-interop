...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `cinterop`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in cinterop.__all__ or not hasattr(cinterop, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in cinterop.__all__:
...
@pytest.mark.skip(reason="This test is currently failing due to discrepancies between the inventory and the API, constrained by legacy package structure.")
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("cinterop")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "cinterop" or item.name.startswith("cinterop."))
        ):
